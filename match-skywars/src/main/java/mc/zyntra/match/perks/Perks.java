...
@Getter
@Setter
public abstract class Perks implements Listener {
    private String name;
    ...
    public Perks(String name, Integer price) {
        setName(name);
        setPrice(price);
    }
    ...
    protected boolean hasAbility(MatchPlayer player, String name) {
        boolean using = false;
        if (player.getPerk().equalsIgnoreCase(name)) {
            using = true;
        }
        return using;
    }
